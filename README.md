...
├── motion_data.csv         # File currently read by src/plot.py
├── motion_data1.csv        # Fresh output generated by src/track_pos.py
├── report.pages
└── src/
...
- Scans injury-angle folders under `data/injury_<id>/<angle_id>/`.
...
- Loads `motion_data.csv` (currently hard-coded in the script).
- Groups by `(injury_id, angle_id)`.
- Plots frame-wise velocity and acceleration trends.

> Note: `src/track_pos.py` writes fresh output to `motion_data1.csv`, while `src/plot.py` reads `motion_data.csv`.  
> To visualize the newest extraction, copy `motion_data1.csv` to `motion_data.csv` (or manually change the CSV path in `src/plot.py`).
...
### Step 3 — Plot results
If you want to plot the latest extraction output:
```bash
cp motion_data1.csv motion_data.csv
```
